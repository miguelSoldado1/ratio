  ...
  return (
    <Sortable getItemValue={(item) => item.id} onValueChange={onSortingChange} value={sorting}>
      <Popover onOpenChange={setOpen} open={open}>
        <PopoverTrigger render={<Button onKeyDown={onTriggerKeyDown} size="sm" variant="outline" />}>
          <ArrowDownUp />
          Sort
          {sorting.length > 0 && (
            <Badge
...
function DataTableSortItem({
  ...
  return (
    <SortableItem asChild value={sort.id}>
      {/* biome-ignore lint/a11y/noNoninteractiveElementInteractions: keyboard shortcut (delete/backspace) to remove the focused sort item */}
      <li className="flex items-center gap-2" id={sortItemId} onKeyDown={onItemKeyDown} tabIndex={-1}>
        <Popover onOpenChange={setShowFieldSelector} open={showFieldSelector}>
          ...
          <PopoverContent className="w-(--anchor-width) p-0" id={fieldListboxId}>
            <Command>
        ...
        >
          ...
          <SelectContent className="min-w-(--anchor-width)" id={directionListboxId}>
