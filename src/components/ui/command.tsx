...
function CommandDialog({
  ...
  return (
    <Dialog {...props}>
      ...
      <DialogContent
        className={cn(
          "data-closed:zoom-out-100 data-open:zoom-in-100 top-18 translate-y-0 overflow-hidden rounded-4xl! p-0 duration-0 data-closed:animate-none data-open:animate-none",
          className
        )}
...
function CommandItem({ className, children, ...props }: React.ComponentProps<typeof CommandPrimitive.Item>) {
  return (
    <CommandPrimitive.Item
      className={cn(
        "group/command-item relative flex cursor-default select-none items-center gap-2 in-data-[slot=dialog-content]:rounded-3xl rounded-2xl px-3 py-2 font-medium text-sm outline-hidden data-[disabled=true]:pointer-events-none in-data-[selection-visible=true]:data-selected:bg-muted data-selected:text-foreground data-[disabled=true]:opacity-50 [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0 [@media(hover:hover)_and_(pointer:fine)]:hover:bg-muted data-selected:*:[svg]:text-foreground",
        className
      )}
